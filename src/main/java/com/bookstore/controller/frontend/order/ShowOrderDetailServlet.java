...
@WebServlet("/show_order_detail") // URL 패턴 "/show_order_detail"에 서블릿을 매핑
public class ShowOrderDetailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // 고객을 위한 주문 상세 정보를 보여주는 메서드 호출
        orderServices.showOrderDetailForCustomer();
    }
    
//    @WebServlet("/show_order_detail"): 이 서블릿 클래스를 "/show_order_detail" URL 패턴에 매핑하는 어노테이션입니다. 클라이언트가 "/show_order_detail" 경로로 GET 방식의 요청을 보내면 이 서블릿이 처리한다는 것을 의미합니다.
//
//    public class ShowOrderDetailServlet extends HttpServlet { ... }: HttpServlet 클래스를 확장하는 ShowOrderDetailServlet 클래스 선언으로, 이 클래스는 HTTP 요청을 처리하는 서블릿으로 동작합니다.
//
//    private static final long serialVersionUID = 1L;: 직렬화된 버전의 고유 식별자로, Serializable 인터페이스를 구현한 클래스의 버전 관리에 사용됩니다.
//
//    public ShowOrderDetailServlet() { super(); }: ShowOrderDetailServlet 클래스의 기본 생성자입니다.
//
...
//
//    orderServices.showOrderDetailForCustomer();: OrderServices 클래스의 showOrderDetailForCustomer() 메서드를 호출하여 고객을 위한 주문 상세 정보를 보여줍니다. 이 메서드는 고객에게 주문에 대한 상세 정보를 표시하는 작업을 수행할 것으로 예상됩니다.
}
